...
--record-overwrite        明示した --record file が存在する場合に置き換える
--record-size-limit BYTES record event 書き込みをこの byte 数で止める。default 0
...
明示した record path が既に存在する場合は、`--record-overwrite` を指定しない
限りエラーにします。

record file size limit はデフォルト 0、つまり無制限です。
`--record-size-limit BYTES` を指定すると、次の event で上限を超える時点で
record event の追記を止め、live probe は継続します。
...
replay、stats、log conversion は session header 後の空行や壊れた JSONL event 行を
skip するため、部分的に壊れた record でも残りの有効な event を利用できます。
