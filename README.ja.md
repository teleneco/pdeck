...
--no-record               config で有効な recording をこの実行だけ無効にする
...
live mode は recording の既定値を `~/.config/pdeck/config.toml` から読み込めます。
record 関連の CLI option を指定した場合は CLI が優先されます。
...
`record = true` にすると live session をデフォルトで記録します。
`record_dir` を省略した場合、config で recording が有効なときは
`~/.config/pdeck/records` を使います。この実行だけ無効化したい場合は
`--no-record` を使います。

デフォルトでは、file なしの `pdeck --record` も `record_dir` 配下へ保存します。
従来どおりカレントディレクトリへ保存したい場合は
`always_use_record_dir = false` を設定します。
...
default config file と records directory がなければ作成:
